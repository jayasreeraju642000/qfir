...
class KnowFundDetailForSmallScreen extends StatefulWidget {
  ...
  KnowFundDetailForSmallScreen(this.model,
      {this.analytics, this.observer, this.responseData, this.tabIndex});

  @override
  State<StatefulWidget> createState() {
    return _KnowFundDetailForSmallScreenState();
  }
}

class _KnowFundDetailForSmallScreenState
    extends State<KnowFundDetailForSmallScreen>
  ...
  void initState() {
    ...
    /* for (int i = 0; i < _benchmarkPerformance['markets'].length; i++) {
			markets.add({
				"value": _benchmarkPerformance['markets'][i].toString(),
				"title": _benchmarkPerformance['markets'][i].toString()
			});
		}
		_selectedMarket = _benchmarkPerformance['markets'][0]; */
  }
  ...
  Widget build(BuildContext context) {
    // changeStatusBarColor(Color(0xffecf1fa));
      ...
      child: ListView(
        children: <Widget>[
          _keyStatsBox1(
              ...
              returns: statsData.containsKey('cagr') ? statsData['cagr'] : null,
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
              ...
              child: statsData['05'] == null && statsData['0andUnder'] == null
                  ? emptyGraph()
                  : HorizontalBarChart(performanceCurrentTabIndex == 0
                      ? _horizontalBarGraphDataBenchmark()
                      : _horizontalBarGraphDataETF())),
          SizedBox(height: getScaledValue(21)),
          Container(
            child: Text("Rolling 1 year fund returns",
              ...
              child: Row(children: <Widget>[
                Expanded(
                    child: _returnComparisionValues("Minimum",
                        roundDouble(statsData['rollretmin'], postFix: "%"),
                        includeLeftBorder: false)),
                Expanded(
                    child: _returnComparisionValues(
                  "Maximum",
                  roundDouble(statsData['rollretmax'], postFix: "%"),
                )),
                Expanded(
                    child: _returnComparisionValues(
                  "Average",
                  roundDouble(statsData['rollretmean'], postFix: "%"),
                )),
              ]))
        ],
      ),
